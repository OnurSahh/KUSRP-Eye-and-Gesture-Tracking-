...
gesture_images = {
    "Fist": pygame.image.load("gesture_Fist.jpg"),
    "OK": pygame.image.load("gesture_OK.jpg"),
    "Rock n Roll": pygame.image.load("gesture_Rock n Roll.jpg"),
    "Palm": pygame.image.load("gesture_Palm.jpg"),
    "Peace Sign": pygame.image.load("gesture_Peace Sign.jpg"),
    "Thumbs Down": pygame.image.load("gesture_Thumbs Down.jpg"),
    "Surfer Sign": pygame.image.load("gesture_Surfer Sign.jpg"),
    "L sign": pygame.image.load("gesture_L Sign.jpg"),
}
...
waiting_text = font.render("Get ready!", True, BLACK)
window.blit(waiting_text, (WINDOW_WIDTH // 2 - waiting_text.get_width() // 2, WINDOW_HEIGHT // 2 - waiting_text.get_height() // 2))
pygame.display.update()
...
while running:
    ...
    if event.key == pygame.K_q:
        running = False
    """
    valid_actions = ["Fist", "OK", "Rock n Roll", "Palm", "Peace Sign", "Thumbs Down", "Surfer Sign", "L sign"]
    ...
    if action1 in valid_actions and action_count > 10:
        last_action1 = str
        if waiting_for_input and not additional_key_required:
            player_sequence.append(action1)
            current_underlines += 1
            placeholders += action1 + " "
    ...
    if displaying_sequence:
        ...
        if len(player_sequence) == len(sequence):
            if additional_key_required:
                additional_text = font.render("Thumbs Up to continue", True, BLACK)
    ...
    if not displaying_sequence and waiting_for_input:
        ...
        for i in range(len(player_sequence)):
            ...
            letter_surface = font.render(str(action_number), True, color)
